...
export default function ProductsCatalogLoadingSkeleton() {
    return (
        <div className="flex-1 container-custom py-4 md:py-6" aria-busy="true">
            ...
            {/* Category Selector Pill Bar Skeleton */}
            <div className="flex items-center gap-2.5 overflow-x-auto scrollbar-hide py-2 mb-6">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div
                        key={i}
                        className="h-10 w-28 shrink-0 rounded-full bg-gray-100 dark:bg-zinc-800 border border-gray-200/50 dark:border-white/5 animate-pulse"
                    />
                ))}
            </div>
            ...
            <div className="flex items-center justify-between gap-4 mb-6 pb-3 border-b border-gray-100 dark:border-white/10">
                <div className="flex flex-col gap-1.5">
                    <div className="w-44 h-7 bg-gray-200 dark:bg-zinc-800 rounded-lg animate-pulse" />
